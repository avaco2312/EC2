...
func main() {
	// Servidor de REST API, GET en localhost:8080/paquete, devuelve status:
	// 200 OK, en el body paquete de lecturas
	// 404 NOT FOUND, paquete no disponible
	// 500 INTERNAL SERVER ERROR, no debe suceder, pero...
	r := mux.NewRouter()
	r.HandleFunc("/paquete", generaPaquete).Methods("GET")
	log.Fatal(http.ListenAndServe(":9000", r))
}

// Genera y sirve un paquete que contiene de 0 a 500 lecturas aproximadamente
func generaPaquete(w http.ResponseWriter, r *http.Request) {
